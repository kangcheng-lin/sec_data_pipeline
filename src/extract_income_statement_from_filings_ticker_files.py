...
def extract_income_statements_from_ticker_folder(
    ticker_dir: Path,
    ticker: str | None = None,
    batch_id: str | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Extract income statements fields from one ticker folder.
    ...
    for path in files:
        ...
        try:
            ...
            print(f"[income-statements extraction failed] {path.name}: {e}")
...
def append_ticker_output(
    ...
    output_dir: Path,
    ticker: str,
    statement_name: str = "income_statement",
) -> Path:
    """
    Save/update one independent CSV per ticker.

    Example output:
        data/processed/sec_income_statement/AAPL_income_statement.csv
...
def extract_income_statements_from_batch_folder(
    input_dir: Path,
    output_dir: Path,
    batch_id: str | None = None,
    ...
    """
    Extract income statement fields from all ticker subfolders
    in one rolling batch folder.

    Writes:
        income_statement_batch_<batch_id>.csv
        income_statement_extraction_status_<batch_id>.csv
        <TICKER>_income_statement.csv for each ticker (append + deduplicate)
        income_statement_all.csv  (optional deduplicated cross-ticker master)
    """
    ...
    for ticker_dir in ticker_dirs:
        ...
        df_ticker, status_ticker = extract_income_statements_from_ticker_folder(
    ...
    batch_output = output_dir / f"income_statement_batch_{batch_label}.csv"
    status_output = output_dir / f"income_statement_extraction_status_{batch_label}.csv"
    ...
    if update_master:
        master_output = output_dir / "income_statement_all.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract income statements fields from SEC filings.")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/processed/sec_income_statement",
        help="Output directory for extracted income statements CSV files.",
    )
    parser.add_argument(
        "--output-csv",
        type=str,
    ...
    parser.add_argument(
        "--no-master",
        action="store_true",
        help="Do not update income_statement_all.csv in batch mode.",
    )
...
if __name__ == "__main__":
    ...
    if args.mode == "single":
        df = extract_income_statements_from_folder(input_dir)
        ...
        extract_income_statements_from_batch_folder(
            input_dir=input_dir,
