...
# ============================================================
# BATCH / ROLLING PIPELINE HELPERS
# ============================================================


def parse_accession_from_filename(path: Path) -> str | None:
    """Extract SEC accession number from our downloaded filename convention."""
...
def compute_net_debt(df: pd.DataFrame) -> pd.DataFrame:
    """Compute netDebt = totalDebt - cashAndCashEquivalents where both exist."""
    if df.empty:
        return df

    if "totalDebt" in df.columns and "cashAndCashEquivalents" in df.columns:
        mask = df["totalDebt"].notna() & df["cashAndCashEquivalents"].notna()
        df.loc[mask, "netDebt"] = df.loc[mask, "totalDebt"] - df.loc[mask, "cashAndCashEquivalents"]

    return df


def extract_balance_sheets_from_ticker_folder(
    ticker_dir: Path,
    ticker: str | None = None,
    batch_id: str | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Extract balance-sheet rows from one ticker folder.

    Expected rolling-batch structure:
        sec_filings_rolling_tmp/batch_00001/AAPL/*.htm
        sec_filings_rolling_tmp/batch_00001/AAPL/*.html
        sec_filings_rolling_tmp/batch_00001/AAPL/*.txt
    """
    ...
    for path in files:
        ...
        try:
            ...
            print(f"[balance-sheet extraction failed] {path.name}: {e}")
...
def append_ticker_output(
    ...
    output_dir: Path,
    ticker: str,
    statement_name: str = "balance_sheet",
) -> Path:
    """
    Save/update one independent CSV per ticker.

    Example output:
        data/processed/sec_balance_sheet/AAPL_balance_sheet.csv
...
def extract_balance_sheets_from_batch_folder(
    input_dir: Path,
    output_dir: Path,
    batch_id: str | None = None,
    ...
    """
    Extract balance sheet fields from all ticker subfolders
    in one rolling batch folder.

    Writes:
        balance_sheet_batch_<batch_id>.csv
        balance_sheet_extraction_status_<batch_id>.csv
        <TICKER>_balance_sheet.csv for each ticker (append + deduplicate)
        balance_sheet_all.csv  (optional deduplicated cross-ticker master)
    """
    ...
    for ticker_dir in ticker_dirs:
        ...
        df_ticker, status_ticker = extract_balance_sheets_from_ticker_folder(
    ...
    batch_output = output_dir / f"balance_sheet_batch_{batch_label}.csv"
    status_output = output_dir / f"balance_sheet_extraction_status_{batch_label}.csv"
    ...
    if update_master:
        master_output = output_dir / "balance_sheet_all.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract balance-sheet fields from SEC filings.")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/processed/sec_balance_sheet",
        help="Output directory for extracted balance-sheet CSV files.",
    )
    parser.add_argument(
        "--output-csv",
        type=str,
    ...
    parser.add_argument(
        "--no-master",
        action="store_true",
        help="Do not update balance_sheet_all.csv in batch mode.",
    )
...
if __name__ == "__main__":
    ...
    if args.mode == "single":
        df = extract_balance_sheets_from_folder(input_dir)
        df = compute_net_debt(df)
        ...
        extract_balance_sheets_from_batch_folder(
            input_dir=input_dir,
